...
const TOOLS = [
  ...
  { type: 'function', function: { name: 'hybrid_search', description: 'Hybrid code search ("grep on steroids"): one call combines exact regex/keyword matching with semantic ranking over a symbol-aware local index — fully offline, no model downloads. Use when you want code that *does* something even if it does not contain the exact words, or to rank grep hits by relevance.', parameters: { type: 'object', properties: { query: { type: 'string', description: 'What to find — keywords, a regex, or a natural-language description' }, mode: { type: 'string', enum: ['hybrid', 'regex', 'keyword', 'semantic'], description: 'hybrid (default): exact + semantic; regex/keyword: exact only; semantic: meaning only' }, path: { type: 'string', description: 'Directory to search (default: project root)' }, limit: { type: 'integer', description: 'Max results (default 10, max 30)' } }, required: ['query'] } } },
...
const COMPOUND_TOOLS = [
  ...
  { type: 'function', function: { name: 'run_tests', description: 'Run the project\'s test suite and return structured results: pass/fail counts and per-failing-test names and messages. Preferred over bare `bash` for test runs — the structured output enables TDD phase transitions and is easier for small models to parse.', parameters: { type: 'object', properties: { test_filter: { type: 'string', description: 'Optional: run only tests whose name matches this string or pattern. Supported by pytest (-k), jest/vitest (-t), go test (-run), cargo test, mocha (--grep), rspec (-e). Use this in the RED phase to confirm a specific new test fails.' } }, required: [] } } },
  { type: 'function', function: { name: 'tdd_loop', description: 'Start a TDD loop for a list of requirements. The harness will enforce Red→Green→Refactor automatically as you write tests and implementation — call run_tests or advance phases yourself. Just write tests and code; the loop advances when tests pass and completes when all requirements are green and the full suite is clean.', parameters: { type: 'object', properties: { requirements: { type: 'array', items: { type: 'string' }, description: 'Behaviours to implement, each as plain English (e.g. "add() returns the sum of two numbers").' } }, required: ['requirements'] } } },
  { type: 'function', function: { name: 'tdd_status', description: 'Show the current TDD phase, requirements checklist, and what to write next.', parameters: { type: 'object', properties: {}, required: [] } } },
];
